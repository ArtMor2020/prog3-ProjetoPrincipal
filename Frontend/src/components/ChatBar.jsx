...
export default function ChatBar() {
  const { user, openChatWindows, openChatWithUser, closeChatWindow, isChatBarVisible, setIsChatBarVisible } = useUser();
  ...
  const fetchFriends = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const { data } = await apiClient.get(`/friendship/friends/${user.id}`);
      const friendPromises = data.map((friendship) => {
        const friendId = friendship.id_user1 === user.id ? friendship.id_user2 : friendship.id_user1;
        return apiClient.get(`/users/${friendId}`);
      });
    ...
    <>
      <div className="fixed bottom-0 right-0 flex items-end z-40">
        {openChatWindows.map((chatUser) => (
          <ChatWindow key={chatUser.id} chatUser={chatUser} onClose={() => closeChatWindow(chatUser.id)} />
        ))}
        <div className="w-64 bg-white rounded-t-lg shadow-xl border border-gray-300">
          <div className="flex justify-between items-center p-3 bg-blue-600 text-white rounded-t-lg cursor-pointer" onClick={() => setIsChatBarVisible(!isChatBarVisible)}>
            <div className="flex items-center">
              <MessageSquare size={20} className="mr-2" />
              <span className="font-bold">Mensagens</span>
            </div>
            {isChatBarVisible ? <ChevronDown size={20} /> : <ChevronUp size={20} />}
          </div>
          {isChatBarVisible && (
            <div className="h-96 flex flex-col">
              <div className="flex-1 overflow-y-auto">
                {loading ? <p className="p-3 text-gray-500 text-center">Carregando...</p> : 
                 friends.length > 0 ? friends.map((friend) => (
                    <div key={friend.id} onClick={() => handleOpenChat(friend)} className="flex items-center justify-between p-2 hover:bg-gray-100 cursor-pointer border-b">
                      <div className="flex items-center">
                  ...
                  )) : <p className="p-3 text-gray-500 text-center">Adicione amigos para conversar.</p>}
              </div>
            </div>
          )}
        </div>
